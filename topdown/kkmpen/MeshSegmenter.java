...
class MeshSegmenter
{
	...
	public Vector KkmSegmentation (int k)
	{
		// Initialization of N, segments, segInfo, numits
		int N = trimesh.nfaces;
		Hashtable segments = new Hashtable ();
		Vector segInfo = new Vector ();
		int numits = 2;

		// Katz/Tal: Compute k maximally geometrically distant segments
System.err.println ("Beginning Katz/Tal geometric segmentation...");
		segments = FuzzySegmentation (k);
System.err.println ("Katz/Tal segmentation complete.");

		// k-means: Repeatedly compute segments w/ topological distance
System.err.println ("Beginning k-means topological EM...");
		segments = KmeansSegmentation (k, numits, segments);
System.err.println ("k-means expectation maximization complete.");

		// Pack segmentation into Vector segInfo
	...
	public Hashtable FuzzySegmentation (int k)
	{
		int N = trimesh.nfaces;
		Hashtable segments = new Hashtable ();
...
System.err.println ("Prob; distances for face " + i + " computed.");
	...
	public Hashtable KmeansSegmentation (int k,
	                                     int numits,
	                                     Hashtable segments)
	{
		...
		// Fill in facesIn with data in Hashtable segments
		for (int fnum = 0; fnum < trimesh.nfaces; fnum++)
		{
			Integer Fnum = new Integer (fnum);
			int cnum = ((Integer)segments.get (Fnum)).intValue ();
			facesIn[cnum].addElement (Fnum);
		}
...
*/
			// Recluster faces using /\ geodesic penalty
			for (int j = 0; j < trimesh.nfaces; j++)
			{
				int f0 = NearestFace (centers[0]);
				int f1 = NearestFace (centers[1]);
				double d0 = GeomDist (j, f0);
				double d1 = GeomDist (j, f1);
				int centersmall = -1, centerbig = -1;
				double mindistsmall = -1.0, mindistbig = -1.0;
				if (d0 < d1)
				{
					mindistsmall = d0;
					mindistbig   = d1;
				...
				for (int m = 2; m < k; m++)
				{
					int f = NearestFace (centers[m]);
					double dist = GeomDist (j, f);
					if (dist < mindistbig)
					{
						if (dist < mindistsmall)
						{
							mindistbig =
							       mindistsmall;
							mindistsmall = dist;
				...
				int fsmall = NearestFace (centers[centersmall]);
				int fbig   = NearestFace ( centers[centerbig] );
				double unbelongsmall = Unbelong (j, fsmall);
				double unbelongbig   = Unbelong ( j, fbig );
				if (unbelongsmall < unbelongbig)
					facesIn[centersmall].addElement
					                     (new Integer (j));
				else
					facesIn[centerbig].addElement
					                   (new Integer (j));
			}
		}

		// Store clustering into Hashtable segments
		segments = new Hashtable (); // Clear out initial clustering
	...
	// Returns geometric distance between centroids of faces fnum and cfnum
	public double GeomDist (int fnum, int cfnum)
	{
		Face tri1 = trimesh.faces[fnum];
		Face tri2 = trimesh.faces[cfnum];
		Point c1 = Centroid (tri1);
		Point c2 = Centroid (tri2);
		return Point.Difference (c1, c2).Length ();
	}

	// Returns geodesic distance between faces fnum and cfnum, computed
	// using Dijkstra's Algorithm
	public double GeoDist (int fnum, int cfnum)
	{
		double[] dist = new double[trimesh.nfaces];
		int[] pred = new int[trimesh.nfaces];
		GetDijkstra (fnum, dist, pred);
		return dist[cfnum];
	}

	// Get distances between face with index _index_ and all other faces
	public void GetDijkstra (int index, double[] dist, int[] pred)
	{
		int N = trimesh.nfaces;
		int i;
	...
	// Computes geodesic curvature-based "unbelongingness" of face fnum
	// to cluster with center point nearest to face cfnum
	public double Unbelong (int fnum, int cfnum)
	{
		if (fnum == cfnum) return 0;
		double[] dist = new double[trimesh.nfaces];
		int[] pred = new int[trimesh.nfaces];
		GetDijkstra (fnum, dist, pred);
		int previous = cfnum;
		int current = pred[cfnum];
		double unbelong = 0.0;
		while (previous != fnum)
		{
			unbelong += 1.0 - Curvature (previous, current);

			// Move on to next pair of faces on geodesic path
			previous = current;
	...
	// Returns curvature of edge connecting face1 and face2 based on
	// normal vectors of face1 and face2
	public double Curvature (int face1, int face2)
	{
		Vec[] normals = new Vec[2];
		TrianglePair pair = new TrianglePair ();
		pair.face1 = face1; pair.face2 = face2;
	...
	// Returns face on triangle mesh with centroid closest to Point p
	public int NearestFace (Point p)
	{
		double mindist = Double.MAX_VALUE;
		int closest = -1;
		for (int fnum = 0; fnum < trimesh.nfaces; fnum++)
		{
			Point centroid = Centroid (trimesh.faces[fnum]);
			Vec diff = Point.Difference (centroid, p);
...
if (closest == -1)
{
System.err.println ("p = " + p.toString () + ", closest = -1");
}
		return closest;
	}
	...
	// Returns geometric distance from triangle index to all other
	// triangles
	public void GetGeomDistances (int index, double[] dist)
	{
		int N = trimesh.nfaces;
		dist[index] = 0.0;	// distance to itself is zero
		for (int i = 0; i < N; i++)
			if (i != index)
				dist[i] = GetDistance (i, index);
	}
