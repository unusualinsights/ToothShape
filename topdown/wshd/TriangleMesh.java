...
class TriangleMesh
{
	...
	{
		final Point p0 = vertices[0].Coord ();
		final double scalefactor = 1.0 / (double)nverts;
		Point centroid = p0;
		for (int i = 1; i < nverts; i++)
		{
			final Point pi = vertices[i].Coord ();
			Vec wpi_p0 = Point.Difference (pi, p0);
			wpi_p0.Scale (scalefactor);
			centroid.Translate (wpi_p0);
		}
		return centroid;
	}
