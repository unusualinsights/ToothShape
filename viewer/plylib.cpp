...
void ply_close(PlyFile *plyfile)
{
  ...
  if ((plyfile->fp != stdin) && (plyfile->fp != stdout))
    fclose (plyfile->fp);
  plyfile->fp = NULL;

  /* free up memory associated with the PLY file */
  for (i = 0; i < plyfile->nelems; i++) {
    free(plyfile->elems[i]->name);
    plyfile->elems[i]->name = NULL;

    for (j = 0; j < plyfile->elems[i]->nprops; j++) {
      free(plyfile->elems[i]->props[j]->name);
      plyfile->elems[i]->props[j]->name = NULL;
      free(plyfile->elems[i]->props[j]);
      plyfile->elems[i]->props[j] = NULL;
    }
    free(plyfile->elems[i]->props);
    plyfile->elems[i]->props = NULL;
    free(plyfile->elems[i]->store_prop);
    plyfile->elems[i]->store_prop = NULL;

    free(plyfile->elems[i]);
    plyfile->elems[i] = NULL;
  }
  free(plyfile->elems);
  plyfile->elems = NULL;
  ...
  if (plyfile->other_elems) {
    for (i = 0; i < plyfile->other_elems->num_elems; i++) {
      free(plyfile->other_elems->other_list[i].elem_name);
      plyfile->other_elems->other_list[i].elem_name = NULL;

      for (j = 0; j < plyfile->other_elems->other_list[i].elem_count; j++) {
/*There are other pieces that should be freed here, but I can't figure them out...*/
        free(plyfile->other_elems->other_list[i].other_data[j]);
        plyfile->other_elems->other_list[i].other_data[j] = NULL;
      }
/*
      free(plyfile->other_elems->other_list[i]);
      plyfile->other_elems->other_list[i] = NULL;
*/
    }
    free(plyfile->other_elems->other_list);
    plyfile->other_elems->other_list = NULL;
